<template>
  ...
  <UiContainer class="py-16 lg:py-24">
    ...
    <div>
      <UiBadge size="lg" variant="secondary">Default</UiBadge>
      <Alert class="border-l-8 border-l-primary" title="New users" description="Over 25K new users joined today!"
        icon="lucide:users" />
    </div>
    <template v-for="(f, i) in features" :key="i">
      <section class="mt-12 grid grid-cols-1 items-center gap-10 lg:mt-24 lg:h-[520px] lg:grid-cols-2 lg:gap-20">
        ...
        <img :src="f.imageUrl" :alt="f.title" class="h-[300px] w-full rounded-lg object-cover shadow-sm lg:h-[520px]" />
      </section>
    </template>
  </UiContainer>
</template>
